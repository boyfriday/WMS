...
export default function Orders() {
  ...
  const handleUpdateStatus = async (id: string, status: string) => {
    try {
      ...
      toast.success(
        `Order status updated to '${statusThaiLabels[status] || status}' successfully!`,
      );
      fetchData();
    } catch (err: any) {
      toast.error(
    ...
    try {
      ...
      toast.error(
        err.response?.data?.message ||
          "Failed to process claim. Make sure you are not claiming more than the remaining purchasable quantity.",
      );
  ...
  const fetchData = async () => {
    ...
    try {
      if (user.role === "Customer") {
        ...
        setProducts(
          (pRes.data.data || []).filter(
            (p) => p.stock > 0 && !p.isDeleted && !p.category?.isDeleted,
          ),
        );
  ...
  const addToCart = (productId: string) => {
    ...
    if (existing) {
      if (existing.quantity >= p.stock) {
        toast.warning(
          `Cannot add more. Only ${p.stock} units available in inventory.`,
        );
        return;
      }
      setCart(
  ...
  const handleSubmit = async () => {
    ...
    if (!selectedCustomerId)
      return toast.warning("Please select a shipping customer");
    try {
            ...
            <div className="lg:col-span-3 space-y-4">
              ...
              <div className="border border-slate-100 rounded-2xl max-h-[360px] overflow-y-auto divide-y divide-slate-100 bg-slate-50/50">
                {filteredProducts.map((p) => (
                  <div
                    key={p.id}
                    className="flex justify-between items-center p-3.5 hover:bg-white transition-colors"
                  >
                    <div>
                ...
                <div className="space-y-3 max-h-[220px] overflow-y-auto pr-1">
                  {cart.map((item) => {
                    ...
                    return (
                      <div
                        key={item.productId}
                        className="flex justify-between items-center bg-white border border-slate-100 p-3 rounded-xl shadow-2xs"
                      >
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-xl max-w-lg w-full mx-4 animate-scale-up">
            ...
            <form onSubmit={handleClaimSubmit} className="space-y-4">
              <p className="text-xs text-slate-500">
                Specify returned quantities for items in this order. Returned
                items will be added back to the inventory stock.
              </p>
              ...
              <div className="border border-slate-100 rounded-xl divide-y divide-slate-100 max-h-[250px] overflow-y-auto bg-slate-50/50">
                {claimOrder.items.map((item) => {
                  ...
                  return (
                    <div
                      key={item.productId}
                      className="flex justify-between items-center p-3 text-xs bg-white"
                    >
                      <div>
                        ...
                        <div className="text-[10px] text-slate-400 mt-0.5">
                          Purchased: {item.quantity} | Returned:{" "}
